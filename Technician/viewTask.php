<?php
    ...
    if($conn){
        ...
        $sql = "select h.STREET, h.BLOCKNO, h.UNITNO, h.POSTALCODE, h.AREA, h.ID as HOMEOWNERID, u.NAME, t.STATUS, t.DESCRIPTION, k.NAME as SERVICETYPE, t.ID
            from users u 
        ...
        if($res) {
            while($row = mysqli_fetch_assoc($res)) {
                ...
                "AREA" => $row['AREA'], "NAME" => $row['NAME'], "STATUS" => $row ['STATUS'], "DESCRIPTION" => $row['DESCRIPTION'], "SERVICETYPE" => $row['SERVICETYPE'], "ID" => $row['ID'], "HOMEOWNERID" => row['HOMEOWNERID']);
            }
